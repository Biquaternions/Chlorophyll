Contributing to Chlorophyll
==========================
Chlorophyll is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Chlorophyll will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Chlorophyll from
...
- A Java 21 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Chlorophyll requires JDK 21 to build, however, makes use of Gradle's
...
Unlike the Chlorophyll API and its implementation, modifications to Paper and Minecraft source files
...
Under `chlorophyll-server`:

- `minecraft-patches` (applies to the `chlorophyll-server//` git repo)
...
3. cd into `chlorophyll-server` for server changes, `chlorophyll-api` for API changes,
   `paper-api` for Paper API changes, and `paper-server` for Paper Server changes.

`chlorophyll-server/src/minecraft/java` and `chlorophyll-server/src/minecraft/java/resources` are not git repositories in the traditional sense.
...
- `./gradlew rebuildPaperSingleFilePatches` - Fixups and rebuilds all paper single-file patches. This is how you'd make changes to the `build.gradle.kts` files located under `chlorophyll-api` and `chlorophyll-server`
...
1. Replace `pick` with `edit` for the commit/patch you want to modify (in this
   case the very first commit, `chlorophyll File Patches`), and
   "save" the changes;
...
optimizations we have. This makes it easier to update Chlorophyll during Minecraft updates,
...
Your commit will be converted into a patch that you can then PR into Chlorophyll.
...
Remember that these changes will affect everyone who runs Chlorophyll, not just you
and your server.
...
- You need to add a comment with a short and identifiable description of the patch:
  `// Chlorophyll start - <COMMIT DESCRIPTION>`
...
- Multi-line changes start with `// Chlorophyll start - <COMMIT DESCRIPTION>` and end
  with `// Chlorophyll end - <COMMIT DESCRIPTION>`.
- One-line changes should have `// Chlorophyll - <COMMIT DESCRIPTION>` at the end of the line.
...
Here's an example of how to mark changes by Chlorophyll:

```java
entity.getWorld().dontBeStupid(); // Chlorophyll - Move away from beStupid()
entity.getFriends().forEach(Entity::explode);
entity.updateFriends();

// Chlorophyll start - Use plugin-set spawn
// entity.getWorld().explode(entity.getWorld().getSpawn());
Location spawnLocation = ((CraftWorld) entity.getWorld()).getSpawnLocation();
entity.getWorld().explode(new BlockPosition(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
// Chlorophyll end - Use plugin-set spawn
```
...
public class SomeVanillaClass {
    public final org.bukkit.Location newLocation; // Chlorophyll - add location
}
```
...
but the visibility is too low (e.g. a private field in an entity class). Chlorophyll can use access transformers
to change the visibility or remove the final modifier from fields, methods, and classes. Inside the `build-data/chlorophyll.at`
...
When submitting feature patches to Chlorophyll, we may ask you to add notes to the patch
...
to make future updates easier on ourselves, Chlorophyll tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Chlorophyll - OBFHELPER
// ...   
this.someMethod(fromX); // Chlorophyll
```
...
```java
if (!ChlorophyllConfig.SETTINGS.SAVE_EMPTY_SCOREBOARDS_TEAM) {
```

It is often preferred that you use the fully qualified name for the
configuration class when accessing it, like so:
`me.biquaternions.chlorophyll.ChlorophyllConfig.VALUE_HERE`.  
If this is not done, a developer for Chlorophyll might fix that for you before
...
To build and install the Chlorophyll APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Chlorophyll repository added, put the local repository above Chlorophyll's.
...
- If you later need to use the chlorophyll-api, you might want to remove the jar
